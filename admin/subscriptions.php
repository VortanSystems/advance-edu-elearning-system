          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-header">
                <h5 class="title">Manage Subscription</h5>
...
<?php
$fetch_courses = mysqli_query($db,"SELECT * FROM transactions ORDER BY pay_date DESC");
...
?>
                      ...
                      <tr>
                        <td>
                          <?php
                          $payer = (htmlentities($courses['owner']));
                          $fetch_payer = mysqli_query($db,"SELECT * FROM advance_users WHERE tutor_ids='$payer'");
                          $payer_tra = mysqli_fetch_array($fetch_payer);
                           echo(htmlentities($payer_tra['fullname']))."<br>";
                           echo(htmlentities($payer_tra['user_phone']))."<br>";
                           echo(htmlentities($payer_tra['user_email']))."<br>";


                           ?>
                        </td>
                        <td>
                          <?php echo number_format(htmlentities($courses['paid_amount']));?> <?php echo (htmlentities($courses['pay_currency']));?>
                        </td>
                        <td>
                          <?php echo date('D d M Y',strtotime(htmlentities($courses['pay_date'])));?>
                        </td>
                        <td >
                          <?php echo date('D d M Y',strtotime(htmlentities($courses['expire_data'])));?>
                        </td>
                        </td>
                        <td >
                         <?php echo (htmlentities($courses['reference']));?>
                        </td>

                        <td >
                          <?php $brox = (htmlentities($courses['status']));

                          if($brox=='complete')
                          {
                            echo "<span class='btn btn-block btn-success'>Complete</span>";
                          }
                          elseif($brox=='revocked')
                          {
                        ...
                        <td>
                          ...
                          if($brox=='complete')
                          {?>
                            <a style="cursor: pointer;" href="subscriptions.php?decline_upgrade=<?php echo htmlentities($courses['id']);?>" class='btn btn-block btn-danger'>Revock</a>
                          <?php }
                          elseif($brox=='revocked')
                          {?>
                            <a style="cursor: pointer;" href="subscriptions.php?accept_upgrade=<?php echo htmlentities($courses['id']);?>" class='btn btn-block btn-success'>Upgrade User</a>
                          <?php }
